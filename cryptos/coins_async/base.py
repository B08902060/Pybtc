...
class BaseCoin:
    ...
    def sign(self, txobj: Union[Tx, AnyStr], i: int, priv,r:int,s:int,v:int) -> Tx:
        """
    ...
    def signall(self, txobj: Union[str, Tx], priv,r:List[int] ,s:List[int] ,v:List[int] ) -> Tx:
        """
    ...
    def get_sigMsg(self, txobj: Union[Tx, AnyStr], i: int ,pub) -> Tx:
        """
        Sign a transaction input with index using a private key
        """
        if not isinstance(txobj, dict):
        ...
        if segwit:
            ...
            if "witness" in txobj.keys():
                ...
                txobj["witness"].append(witness)
        return txobj
    def get_TxSigMsh(self,txobj: Union[str, Tx],pub) -> Tx:
        if not isinstance(txobj, dict):
            txobj = deserialize(txobj)
        for i in range(len(txobj["ins"])):
            txobj = self.get_sigMsg(txobj, i, pub)
        return txobj
